...
function resolveImageUrl(item) {
  ...
  if (n === 'tortas' && item.description && item.description.length) {
    const d = normalize(item.description[0]);
    if (d.includes('frango')) return SUPABASE_BASE + 'Torta Franco Requeijao.png';
    if (d.includes('palmito')) return SUPABASE_BASE + 'Torta Palmito.png';
  }

  const extra = {
    'logo centralizado': 'Logo Centralizado.png',
    'lazanha': 'Lazanha.png',
    'lasanha': 'Lazanha.png'
  };
...
function renderCategories() {
  ...
  menuData.forEach(category => {
    ...
    h3.textContent = category.subtitle;
    header.append(h2, h3);
    ...
    category.items.forEach(item => {
      ...
      img.onerror = () => { img.src = SUPABASE_LOGO_URL; };
      ...
      a.href = buildWhatsAppLink(item);
      a.target = '_blank';
      a.rel = 'noopener';
      a.textContent = 'clique aqui e reserve';
      reserve.appendChild(a);
...
function initLogos() {
  ...
  if (headerLogo) {
    headerLogo.src = SUPABASE_LOGO_URL;
    headerLogo.onerror = () => { headerLogo.src = SUPABASE_LOGO_URL; };
  }
  const footerLogo = document.getElementById('footer-logo');
  if (footerLogo) {
    footerLogo.src = SUPABASE_LOGO_URL;
    footerLogo.onerror = () => { footerLogo.src = SUPABASE_LOGO_URL; };
  }
}
